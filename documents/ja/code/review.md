...
* [禁止事項・アンチパターン](./review.md#%E7%A6%81%E6%AD%A2%E4%BA%8B%E9%A0%85%E3%82%A2%E3%83%B3%E3%83%81%E3%83%91%E3%83%BF%E3%83%BC%E3%83%B3)
...
* **マジックナンバーの直接入力**
  - 意味を持つ数値や文字列は定数（const）として定義し、名前を付けて使用すること。
* **関数・クラスの肥大化**
  - 1つの関数が100行を超える、あるいは1つのクラスに責任が多すぎる（単一責任原則に反する）場合は、分割を検討すること。
* **デバッグコードの混入**
  - `console.log` や一時的なコメントアウト、未使用の変数が残っていないこと。
* **感情的なレビュー**
  - レビューは「コード」に対して行うものであり、「人格」を否定するような表現は避けること。
  - 攻撃的な言葉遣いをせず、建設的なフィードバックを心がけること。
